      ...
      {onNavigateHome && (
        <div className="flex items-center justify-between bg-white border border-slate-200 px-4 py-2.5 rounded-lg shadow-2xs text-xs">
          <div className="flex items-center gap-2 text-slate-500 font-medium">
            ...
            <span>›</span>
            <span className="text-slate-800 font-semibold">Evolução da Criticidade</span>
          </div>
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-5 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-500">Monitoramento Contínuo de Risco Residual</span>
          </div>
        ...
        <div className="flex items-center gap-4 bg-slate-50 border border-slate-200 p-3 rounded-lg">
          <div className="text-center px-2">
            <span className="text-[10px] text-slate-500 font-semibold uppercase block">Score Inicial</span>
            <span className="text-xl font-extrabold text-slate-900">{project.initialScore} pts</span>
          ...
          <div className="text-center px-2">
            <span className="text-[10px] text-slate-500 font-semibold uppercase block">Score Residual Atual</span>
          ...
          <div className="text-center px-2 bg-emerald-100 rounded-md py-1 border border-emerald-300">
            <span className="text-[10px] text-emerald-800 font-bold uppercase block">Queda de Risco</span>
            <span className="text-lg font-black text-emerald-900">-{percentReduction}%</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-6 shadow-xs space-y-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border-b border-slate-200 pb-3">
          ...
          <div className="flex items-center gap-3 text-xs">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-slate-700 font-medium">Pontuação de Risco</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded-full bg-emerald-500 inline-block"></span>
              <span className="text-slate-700 font-medium">Meta de Governança (≤ 5 pts)</span>
            </div>
          </div>
        </div>
              ...
              <Tooltip
                content={({ active, payload, label }) => {
                  if (active && payload && payload.length) {
                    ...
                    return (
                      <div className="bg-slate-900 text-white p-3 rounded-md shadow-lg text-xs space-y-1 border border-slate-800">
                        <div className="font-bold text-slate-200">{data.label} ({data.date})</div>
                        <div className="text-indigo-300 font-extrabold text-sm">
                          Score: {data.score} pontos ({data.status})
                        </div>
                        <div className="text-slate-400 text-[11px] max-w-xs">{data.event}</div>
                      </div>
                    );
                  }
                  return null;
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-2">
          <div className="p-3 bg-slate-50 border border-slate-200 rounded-md text-xs">
            <span className="text-[11px] font-bold text-slate-500 uppercase block">1. Fase de Contenção (Concluída)</span>
            <div className="font-bold text-slate-900 mt-0.5">Backup + Homologação + Sustentação</div>
          ...
          <div className="p-3 bg-blue-50 border border-blue-200 rounded-md text-xs">
            <span className="text-[11px] font-bold text-blue-700 uppercase block">2. Fase Atual (Em Andamento)</span>
            <div className="font-bold text-blue-950 mt-0.5">Segurança & Controle de Acessos</div>
            <p className="text-blue-900 mt-1 text-[11px]">
          ...
          <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-md text-xs">
            <span className="text-[11px] font-bold text-emerald-700 uppercase block">3. Fase de Homologação Final</span>
            <div className="font-bold text-emerald-950 mt-0.5">CI/CD GitHub & Conformidade LGPD</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-6 bg-white border border-slate-200 rounded-lg p-5 shadow-xs space-y-4">
          ...
          <div className="p-3 bg-slate-50 rounded-lg border border-slate-200 flex items-center justify-between">
            <div>
              <span className="text-[11px] font-bold text-slate-500 uppercase block">Score Simulado</span>
              <div className="flex items-baseline gap-2">
                <span className="text-2xl font-black text-slate-900">{effectiveSimScore} pts</span>
                {extraSimPoints > 0 && (
            ...
            <div className={`px-3 py-1.5 rounded-md text-xs font-bold ${riskColor.badge}`}>
